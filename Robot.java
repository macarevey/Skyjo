...
public class Robot {
	...
	public Card getDeckCardPulled() {
		return deckCardPulled;
	}
	
	public Card getDiscardCardPulled() {
		return discardCardPulled;
	}
	...
	public void makeDecision() {
		...
		if (gameState.equals("Normal")) {
			...
			for (int i = 0; i < hand.getHand().size(); i++) { // Find the highest card inside robots hand, only ones
																// that are turned up
			...
			if (deckCardPulled.getNum() < highestCard) { // Deck card is less than the highest
				if ((highestCard >= 7 && highestCard <= 9 && deckCardPulled.getNum() <= 4)
						|| (highestCard <= 5 && highestCard >= 2 && deckCardPulled.getNum() <= 0)
						|| (deckCardPulled.getNum() >= 4 && highestCard >= 10)) {
					Card handCard = hand.getHand().get(highestCardIndex);
					hand.replaceCard(handCard, deckCardPulled);
					discard.addCard(handCard);
					gameState = "Normal";
					deckCardPulled = null;
					doneWithTurn = true;
					...
					gameState = "FlipCard";
					deckCardPulled = null;
				} else { // Pick a random card to replace
			...
			if (discardCardPulled.getNum() < highestCard) { // Deck card is less than the highest
				if ((highestCard >= 7 && highestCard <= 9 && discardCardPulled.getNum() <= 4)
						|| (highestCard <= 5 && highestCard >= 2 && discardCardPulled.getNum() <= 0)
						|| (discardCardPulled.getNum() >= 4 && highestCard >= 10)) {
					Card handCard = hand.getHand().get(highestCardIndex);
					hand.replaceCard(handCard, discardCardPulled);
					discard.addCard(handCard);
					gameState = "Normal";
